// Based on bootstrap-spinedit.js, but modified to not auto-append the spinner template.
// Instead, you must pass two options which are functions that take the spinner input element as an argument
// and should return the increment and decrement action elements.
//
// eg.
//
//   $('.order-popup-qty').spinedit({
//     minimum: 1,
//     incLocator: function(el) {
//       $(el).parent().siblings('.icon-pos')
//     },
//     decLocator: function(el) {
//       $(el).parent().siblings('.icon-neg')
//     },
//   })
...
!function ($) {
    ...
    var value = $.fn.spinedit.defaults.value;
      ...
      if (this.element.val()) {
        ...
        if (hasOptions && typeof options.incLocator == 'function')
          $(options.incLocator(this.element).mousedown($.proxy(this.increase, this))).disableTextSelect();
        if (hasOptions && typeof options.decLocator == 'function')
          $(options.decLocator(this.element).mousedown($.proxy(this.decrease, this))).disableTextSelect();
